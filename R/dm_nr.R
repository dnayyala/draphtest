dm.nr <- function(x, mu = NULL, alpha = NULL, tol = 1e-4, maxiters = 1e2){
    ...
    while (eps > tol & n.step <= maxiters){
        ...
        Q  <- solve(dm.hessian(x, mu=mu1[n.step-1], alpha=alpha1[[n.step-1]], param = "alpha"), dm.jacobian(x, mu=mu1[n.step-1], alpha=alpha1[[n.step-1]], param = "alpha"))
        ...
        W  <- dm.jacobian(x, mu=mu1[n.step - 1], alpha=alpha1[[n.step]], param = "mu")/dm.hessian(x, mu=as.numeric(mu1[n.step - 1]), alpha=alpha1[[n.step]], param = "mu")
