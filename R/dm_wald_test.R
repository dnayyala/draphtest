dm.wald <- function(x, y = NULL, mu.x, mu.y = NULL, alpha.null = NULL, alpha.x, alpha.y = NULL,  type = "one"){
  ...
  if (type == "one"){
    ...
    H.x      <- dm.hessian(x, mu.x, alpha.x, param = "alpha") # Fisher information matrix of data x
    H.y      <- dm.hessian(y, mu.y, alpha.y, param = "alpha") # Fisher information matrix of data x
