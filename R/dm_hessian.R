dm.hessian <- function(x, mu, alpha, param = "alpha"){
  ...
  #' @return If ```param``` is set as ```alpha```, the function will return a $(p-1) x (p-1)$ Hessian matrix, else if ```param``` is set as ```mu```, the function will return a scalar of Hessian
