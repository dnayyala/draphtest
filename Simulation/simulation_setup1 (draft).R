...
####################################################
rm(list=ls())
setwd("/Users/bchoi/Google Drive/Projects/Random Project/Reparameter/R codes/New theta/Dirichlet Multinomial")
...
n        <- 20 ##as.integer(args[2])    # the number of samples from Dirichlet distribution
k        <- 10 #as.integer(args[3])
...
avg.p.value = foreach(i=1:n.boots, .packages=c('gtools', 'ICSNP'), .combine='rbind') %dorng%  {
    ...
    for(m in 1:m.random){
        ...
        raw.raptt  <- (RP.orth %*% x.boot) 
        rx.raptt   <- adj.func(raw.raptt, X.plus) # fix the projected data 
...
## WRITE THE SAVE FILE NAME INCLUDING THE VALUES PROVIDED IN ARGS
## SOMETHING LIKE "Result_args[1]_args[2]_args[3].RData"
#save.image(file=paste0("DM_OneSample_alpha_p_", p,  "_n_", n, "_k_", k,  "_mu_", mu, ".RData"))
#write.csv(emp.alpha, file=paste0("DM_OneSample_alpha_p_", p,  "_n_",  n, "_k_",  k, "_mu_", mu, ".csv"))
